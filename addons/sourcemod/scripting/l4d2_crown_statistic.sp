...
public OnPluginStart()
{
	...
	cvar_welcomemsg = CreateConVar("l4d2_crownstat_welcomemsg", "0", "Witch crown statistic - welcome message enable/disable", CVAR_FLAGS);
...
public Action:Event_Witch_Killed(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		//miss message
		PrintToChat(client, "You killed the witch, but did not crown her!");
	...
	//print a message
	PrintToChat(client, "You crowned the witch! You have %d crowneds on the server", crowns);
...
PrintCrownsToClient(client)
{
	...
	else PrintToChat(client, "You not have crowns on this server", crowns);
}
...
PrintTopCrownersToClient(client)
{
	...
	SetPanelTitle(TopCrownPanel, "Mejores 5 Crowners");
	...
	//add total crowns
	DrawPanelText(TopCrownPanel, "-------------------");
	Format(text, sizeof(text), "Hay %d crowns en el servidor", totalcrowns);
	DrawPanelText(TopCrownPanel, text);
