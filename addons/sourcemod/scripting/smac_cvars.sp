...
// cvar data
Handle
	g_hCvarTrie = INVALID_HANDLE,
	g_hCvarADT  = INVALID_HANDLE;
int
	g_iADTSize;
...
{
	g_bsourcecomms  = LibraryExists("sourcebans++");
	...
	if (SMAC_CheatDetected(client, Detection_CvarViolation, info) == Plugin_Continue)
	{
		...
		switch (CAction)
		{
			case Action_Mute:
			{
				...
				if (g_bsourcecomms && SourceComms_GetClientMuteType(client) != bSess)
				{
					char buffer[64];
					Format(buffer, sizeof(buffer), "%t%t", "SMAC_Tag", "SMAC_Muted");
					SourceComms_SetClientMute(client, true, -1, true, buffer);
				}
				else if (g_bmaterialbans && MAGetClientMuteType(client) != 1)
				{
					MAGetClientMuteType(client);
				}
				else if (g_bbasecomm && !BaseComm_IsClientMuted(client))
				{
