...
ConVar
	...
	g_hCvarCompat = null;
float
	g_fDetectedTime[MAXPLAYERS + 1];
bool
	g_bInMinigun[MAXPLAYERS + 1],
	g_bPrevAlive[MAXPLAYERS + 1];
int
	g_iPrevButtons[MAXPLAYERS + 1]                               = { -1, ... },
								g_iPrevCmdNum[MAXPLAYERS + 1]    = { -1, ... },
								g_iPrevTickCount[MAXPLAYERS + 1] = { -1, ... },
								g_iCmdNumOffset[MAXPLAYERS + 1]  = { 1, ... };
ResetStatus
	g_TickStatus[MAXPLAYERS + 1];
bool
	g_bLateLoad = false;
EngineVersion
	g_EngineVersion = Engine_Unknown;
EngineGroup
	g_EngineGroup = Group_Ignore;
                      ...
                      int& subtype, int& cmdnum, int& tickcount, int& seed, int mouse[2])
{
	...
	switch (g_Game)
	{
		...
		case Game_L4D:
		{
			...
			if (GetClientTeam(client) != L4D2Team_Survivor || L4D_IsSurvivorBusy(client))
			{
				return Plugin_Continue;
			}
		}
		...
		{
			// Only check survivors in first-person view.
			if (GetClientTeam(client) != L4D2Team_Survivor || L4D2_IsSurvivorBusy(client))
			{
				return Plugin_Continue;
			}
		}
