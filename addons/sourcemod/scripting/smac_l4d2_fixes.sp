...
public Action OnPlayerRunCmd(int client, int& buttons, int& impulse, float vel[3], float angles[3], int& weapon, int& subtype, int& cmdnum, int& tickcount, int& seed, int mouse[2])
{
	...
	if ((buttons & IN_ATTACK) && (buttons & IN_ATTACK2) && GetClientTeam(client) == L4D2Team_Infected && IsPlayerAlive(client) && GetEntProp(client, Prop_Send, "m_zombieClass") == L4D2Infected_Tank)
	{
		buttons ^= IN_ATTACK2;
	}
